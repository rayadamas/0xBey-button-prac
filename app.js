...
// load init item on window obj
window.addEventListener('DOMContentLoaded', function () {
  showPerson(currItem);
  // console.log('Big YERR vibes'); //fires when HTML doc has been fired + loaded up
});
...
nextBtn.addEventListener('click', function () {
  ...
  showPerson(currItem);
  console.log(currItem, img, author, job, info);
});
...
randBtn.addEventListener('click', function () {
  currItem = Math.floor(Math.random() * reviews.length); // choosing a rand person based on the amt we have to choose from
  showPerson(currItem);
  console.log(currItem, img, author, job, info);
})
